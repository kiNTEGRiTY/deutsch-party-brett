...
import { GameController } from './engine/game-controller.js?v=arcade-stage-paper-48';
...
class App {
  ...
  async _mountStartPreview() {
    ...
    try {
      const { mountStartPreview } = await import('./ui/render-start-preview.js?v=arcade-stage-paper-48');
  ...
  _applyDebugRoute() {
    ...
    window.setTimeout(async () => {
      const { getMinigame } = await import('./minigames/minigame-registry.js?v=arcade-stage-paper-48');
  ...
  async _getSetupRendererClass() {
    if (!this._setupRendererClass) {
      const module = await import('./ui/render-setup.js?v=arcade-stage-paper-48');
  ...
  async _getBoardRendererClass() {
    if (!this._boardRendererClass) {
      const module = await import('./ui/render-board.js?v=arcade-stage-paper-48');
  ...
  async _getMinigameRendererClass() {
    if (!this._minigameRendererClass) {
      const module = await import('./ui/render-minigame.js?v=arcade-stage-paper-48');
  ...
  async _getMinigameMenuRendererClass() {
    if (!this._minigameMenuRendererClass) {
      const module = await import('./ui/render-minigame-menu.js?v=arcade-stage-paper-48');
